...
def createfile_mean(filename):
    ...
    v = f.createVariable('theta','f4',('z_hybrid_height','latitude',
                                   'longitude',),zlib=True) #**#
    f.close()
...
def createfile(filename):
    ...
    fieldfile = 'xjanpa.pb'+filename[-17:-9] #**#
...
def var_save(dates):
    ...
    v_files = []
    for date in dates: 
        v_files.append('/group_workspaces/jasmin/hiresgw/xjanp/xjanpa.pb'\
                        +date) #**#
    ...
    for i in xrange(len(v_files)):
        ...
        if date[-2:] == '21':
            ...
            appendv_mean = fmean.variables['theta']#**#
            appendv_mean[:] = mean[:]
...
def main():
    ...
    test = False
    if test: 
        infile = '/group_workspaces/jasmin/hiresgw/mj07/xjanpa.pb19920801'
    else:
        infiletemp = argv[1:]
        infile = infiletemp[0]
    ...
    for date in dates:
        createfile(path+'xjanpa.'+date+'.theta.nc')#**#
    path_mean = '/group_workspaces/jasmin/hiresgw/mj07/monthly_means/temp_files/'
    createfile_mean(path_mean+'xjanpa.'+str(infile[-8:-2])+'.theta.nc')#**#
    # send off calcs
    var_save(dates)
