...
def createfile_mean(filename):
    ...
    v = f.createVariable('u','f4',('z_hybrid_height','latitude',
                                   'longitude',),zlib=True) #**#
    f.close()
...
def createfile(filename):
    ...
    fieldfile = 'xjlefa.pj'+filename[-13:-5] #**#
    f2 = cdms2.open('/group_workspaces/jasmin/hiresgw/xjlef/'+fieldfile)#**#
    var = f2.getVariables()    
    v2 = var[9]#**#
...
def var_save(dates):
    ...
    v_files = []
    for date in dates: 
        v_files.append('/group_workspaces/jasmin/hiresgw/xjlef/xjlefa.pj'\
                        +date) #**#
    ...
    for i in xrange(len(v_files)):
        ...
        f = Dataset('/group_workspaces/jasmin/hiresgw/mj07/xjlefa.'\
                    +date+'.u.nc','a') #**#                          
        ...
        if date[-2:] == '21':
            ...
            meanfile = '/group_workspaces/jasmin/hiresgw/mj07/'+\
                'monthly_means/temp_files/xjlefa.'+str(date[:-2])+\
                '.u.nc' #**#                            
...
def main():
    ...
    test = False
    if test: 
        infile = '/group_workspaces/jasmin/hiresgw/mj07/xjlefa.pj19810901'
    else:
        infiletemp = argv[1:]
        infile = infiletemp[0]
    ...
    for date in dates:
        createfile(path+'xjlefa.'+date+'.u.nc')#**#
    path_mean = '/group_workspaces/jasmin/hiresgw/mj07/monthly_means/temp_files/'
    createfile_mean(path_mean+'xjlefa.'+str(infile[-8:-2])+'.u.nc')#**#
    # send off calcs
    var_save(dates)
