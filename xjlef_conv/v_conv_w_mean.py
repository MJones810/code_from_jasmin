...
def createfile_mean(filename):
    ...
    zres = 180
    latres = 325 ### 325 for v, 324 for u
    lonres = 432
    # Create dimensions
...
def createfile(filename):
    ...
    fieldfile = 'xjlefa.pi'+filename[-13:-5] #**#
...
def var_save(dates):
    ...
    v_files = []
    for date in dates: 
        v_files.append('/group_workspaces/jasmin/hiresgw/xjlef/xjlefa.pi'\
                        +date) #**#
...
def main():
    ...
    for date in dates:
        createfile(path+'xjlefa.'+date+'.v.nc')#**#
    path_mean = '/group_workspaces/jasmin/hiresgw/mj07/monthly_means/temp_files/'
    createfile_mean(path_mean+'xjlefa.'+str(infile[-8:-2])+'.v.nc')#**#
    # send off calcs
    var_save(dates)
