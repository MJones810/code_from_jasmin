...
def create_temp_nc(filename):
    ...
    if filename[-3:] == '.nc' :
        tempdir = filename[:-26]+'monthly_means/temp_files/'
        tempfile = filename[-26:-9]+'.temp.nc'
...
def final_average(n):
    ...
    months = glob(directory+'monthly_means/temp_files/xjanpa.pb??????.temp.nc')
    months.sort()
    # Example file to take lat, lon and height vals from
    filename = directory+'xjanpa.pb19910301.theta.nc'
...
def average_to_month(filein):
    ...
    files = glob(directory+'temp_files/'+filein[-26:-11]+'??.temp.nc')
    fmon = Dataset(files[0],'r')
    u = fmon.variables['theta']
    mean = u[:]
    fmon.close()
    
    for filename in files[1:]:
        fmon2 = Dataset(filename,'r')
        u = fmon2.variables['theta']
        mean = (mean+u[:])/2
        fmon2.close()
...
files = glob('/group_workspaces/jasmin/hiresgw/mj07/xjanpa.pb*')
files.sort()
months = glob('/group_workspaces/jasmin/hiresgw/mj07/xjanpa.pb??????01.theta.nc')
months.sort()
