...
def create_nc(filein):
    ...
    bounds_longitude = f.createVariable('bounds_longitude','f8',
                                        ('longitude','bound',))
    u = f.createVariable('p','f4',('time','z_hybrid_height','latitude',
                                   'longitude',),zlib=True)
    ...
    u.lookup_source = 'defaults (cdunifpp V0.14pre1)'
    u.standard_name = 'air_pressure'
    u.long_name = 'AIR PRESSURE'
    u.units = 'Pa'
...
def final_average(n):
    ...
    months = glob(directory+'monthly_means/temp_files/xjanpa.ph??????.temp.nc')
    months.sort()
    # Example file to take lat, lon and height vals from
    filename = directory+'xjanpa.ph19910301.p.nc'
...
files = glob('/group_workspaces/jasmin/hiresgw/mj07/xjanpa.ph*')
files.sort()
months = glob('/group_workspaces/jasmin/hiresgw/mj07/xjanpa.ph??????01.p.nc')
months.sort()
